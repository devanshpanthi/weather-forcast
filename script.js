...
function hideSuggestions() { $("#search-suggestions").hidden = true; }
...
function displaySuggestions(places) {
  ...
  places.forEach((place) => {
    ...
    option.addEventListener("click", () => {
      $("#city-input").value = place.display_name;
      hideSuggestions();
      loadWeather(place.display_name);
    });
...
async function searchSuggestions(query) {
  if (query.trim().length < 3) return hideSuggestions();
  const params = new URLSearchParams({
    q: `${query}, India`, format: "jsonv2", addressdetails: "1", limit: "5", countrycodes: "in", "accept-language": "en"
  });
  try {
    const response = await fetchWithTimeout(`${GEO_URL}?${params}`);
...
$("#city-input").addEventListener("input", (event) => {
  ...
  suggestionTimer = setTimeout(() => searchSuggestions(event.target.value), 1100);
});
$("#city-input").addEventListener("blur", () => setTimeout(hideSuggestions, 180));
